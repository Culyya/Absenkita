<template>
  <div class="py-[3rem]" style="animation: muncul-kiri-kanan-fade 1s;">
    <div class="flex flex-row justify-between font-pop mb-5">
      <h1 class="font-medium text-[20px]">Riwayat Kehadiran</h1>
      <button
     ...
     >
      ...
      <tbody>
        <tr
          ...
          v-for="(abse1,index) in absendata2" :key="abse1.absenId" >
          <td class="px-3 py-4">{{ index + 1 }}</td>
          <td scope="row" class=" flex flex-row px-4 py-4">
            <h2>   {{ formatDate(abse1.jammasuk) }}</h2>
            <p class="mr-2 ml-2">To</p>
            <h2>{{ formatDate(abse1.JamKeluar) }}</h2>
          </td>
          <td class="px-6 py-4 text-center">{{ abse1.status }}</td>
          <td class="px-6 py-4 text-center">{{ abse1.Nomorinduk }}</td>
          <td class="px-6 py-4 text-center">{{ abse1.Nomorinduk }}</td>
        </tr>
      </tbody>
    </table>
...
applyFilter() {
  this.getabsen(this.selectedMonth,this.selectedYear)
    },
    openModal() {
    ...
    getabsen(bulan,tahun) {
        ...
        window.location.reload()
         ...
         getBetween() {
      axios
    .post(`http://${window.location.hostname}${process.env.VUE_APP_BACKEND_HOST}/absen/view`, {
      startDate: this.selectedDate1,
      endDate: this.selectedDate2,
    })
    .then((response) => {
      this.absendata2 = response.data;
    })
    .catch((error) => {
      console.log(error);
