<template>
    <div class="flex flex-col justify-center font-pop">
      <div class=" mb-10">
        <h1 class="text-[24px] font-medium">Data Posisi</h1>
      </div>
    ...
    <transition>
      <div class="fixed z-10 left-0 top-0 w-full h-full flex items-center justify-center overflow-auto bg-black/40 bg-opacity-50  font-pop font-regular" v-if="open">
            <div
        class=" bg-white   px-16 py-4 rounded-lg shadow-lg"
      >
      <form @submit.prevent="submitForm">
        <div class="w-full ">
          <h1 class="text-[24px] mb-4">Tambah Posisi</h1>
          <hr class="mb-4" />
          <div class="flex flex-row justify-between">
           
            <div class="flex flex-col">
              <div class="grid grid-flow-row-dense grid-cols-2">
                <label class="mt-5" for="">Nama Posisi</label>
                <div class="col-span-2">
                  <input
                    class="w-[15rem]  mt-4 mb-4 py-2 border-gray-300 rounded-md"
                    type="text"
                    name="posisi"
                    placeholder="Enter Your Add Posisi User..."
                    autocomplete="off"
                    v-model="nama_posisi"
                  />
                </div>
          ...
          <button class=" mt-5 bg-[#606691] px-5 py-3 rounded-md" @click="open = false">
            ...
            <button class=" ml-4 mt-5  bg-[#82C97C] px-[3rem] py-3 rounded-md"  type="submit">
  ...
  <script>
import TablePosisi from "./TablePosisi.vue";
  ...
  export default {
    ...
    methods: {
      ...
      return;
        ...
        const response = await axios.post(`http://${window.location.hostname}${process.env.VUE_APP_BACKEND_HOST}/posisi`, {
          nama_posisi: this.nama_posisi,
        });
        console.log(response);
        this.message = "Data Posisi berhasil Ditambahkan";
        this.open=false
        this.open1=true
        setTimeout(() => {
        this.open1=false
