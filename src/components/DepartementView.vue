<template>
    <div class="flex flex-col justify-center font-pop">
      <div class=" mb-10">
        <h1 class="text-[24px] font-medium">Data Departement</h1>
      </div>
    ...
    <transition>
      ...
      <form @submit.prevent="submitForm">
        <div class="w-full ">
          <h1 class="text-[24px] mb-4">Tambah Departement</h1>
          <hr class="mb-4" />
          <div class="flex flex-row justify-between">
           
            <div class="flex flex-col">
              <div class="grid grid-flow-row-dense grid-cols-2">
                <label class="mt-5" for="">Nama Departement</label>
                <div class="col-span-2">
                  <input
                    class="w-full  mt-4 mb-4 py-2 border-gray-300 rounded-md"
                    type="text"
                    name="departement"
                    placeholder="Enter Your Add Departement User..."
                    autocomplete="off"
                    v-model="nama_departement"
                  />
                </div>
              </div>
              </div>
  ...
  <script>
import TableDp from "./TableDp.vue";
  ...
  export default {
    ...
    methods: {
      ...
      return;
        ...
        const response = await axios.post(`http://${window.location.hostname}${process.env.VUE_APP_BACKEND_HOST}/departement`, {
          nama_departement: this.nama_departement,
        });
        console.log(response);
        this.message = "Data Departement berhasil Ditambahkan";
        this.open=false
        this.open1=true
        setTimeout(() => {
        this.open1=false
